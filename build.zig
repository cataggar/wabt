...
pub fn build(b: *std.Build) void {
    ...
    //
    // Compile-time bake the adapter bytes into the wabt CLI so
    // `wabt component new` can auto-splice preview1 cores without
    // requiring `--adapt wasi_snapshot_preview1=<path>`. We stage a
    // generated `builtin.zig` next to the adapter wasm in a single
    // WriteFile dir; `@embedFile` then resolves to the run-tool
    // output, which makes Zig pull in the adapter wasm as a build
    // dependency of the CLI exe automatically.
    const builtin_stage = b.addWriteFiles();
    const builtin_zig = builtin_stage.add(
        "builtin.zig",
        "//! Generated by build.zig; do not edit.\n" ++
            "//! Compile-time-embedded wasi-preview1 → preview2 adapter.\n" ++
