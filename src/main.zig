...
// `store.*` import wrappers + the `Pet` / `Toy` types. The module name matches
// the world (`store_consumer`); `.store` is the interface namespace
// (`store.petCount()`, …) and `Pet` is one of the module's types.
