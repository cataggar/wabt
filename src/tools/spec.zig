...
//!
//! The `spec` subject covers WebAssembly spec testing: converting
//! .wast assertion files to JSON commands, and structural validation
//! of .wast inputs.
...
pub const usage =
    ...
    \\
    \\wabt does not execute WebAssembly. To run a .wast against an engine,
    \\convert it with `to-json` and use a runtime such as cataggar/wamr.
    \\
...
test "parseVerb rejects unknown spec verbs" {
    ...
    try std.testing.expectEqual(@as(?Verb, null), parseVerb("run"));
