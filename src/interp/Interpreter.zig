...
fn intType(comptime signedness: std.builtin.Signedness, comptime bits: u16) type {
    if (@hasDecl(std.meta, "Int")) return std.meta.Int(signedness, bits);
...
pub const Interpreter = struct {
    ...
    fn extractLane(self: *Interpreter, comptime T: type, lane: u8, comptime signed: bool) TrapError!void {
        ...
        if (@sizeOf(T) <= 4) {
            if (signed) {
                const s: i32 = @as(i32, @as(intType(.signed, @bitSizeOf(T)), @bitCast(lanes[lane])));
    ...
    fn simdCmpOp(self: *Interpreter, comptime T: type, comptime op: fn (T, T) bool) TrapError!void {
        const Unsigned = intType(.unsigned, @bitSizeOf(T));
        const count = 16 / @sizeOf(T);
        const b: [count]T = @bitCast((try self.popValue()).v128);
        const a: [count]T = @bitCast((try self.popValue()).v128);
        var result: [count]Unsigned = undefined;
        for (0..count) |i| result[i] = if (op(a[i], b[i])) @as(Unsigned, @bitCast(@as(intType(.signed, @bitSizeOf(T)), -1))) else 0;
        try self.pushValue(.{ .v128 = @bitCast(result) });
    }
    ...
    fn simdShiftOp(self: *Interpreter, comptime T: type, comptime dir: enum { left, right_s, right_u }) TrapError!void {
        ...
        const Unsigned = intType(.unsigned, bits);
        const Signed = intType(.signed, bits);
    ...
    fn intAbs(comptime T: type) fn (T) T {
        return struct {
            fn f(a: T) T {
                const Signed = intType(.signed, @bitSizeOf(T));
    ...
    fn intAvgrU(comptime T: type) fn (T, T) T {
        return struct {
            fn f(a: T, b: T) T {
                const Wide = intType(.unsigned, @bitSizeOf(T) * 2);
    ...
    fn floatMin(comptime T: type) fn (T, T) T {
        return struct {
            fn f(a: T, b: T) T {
                ...
                // -0 < +0
                if (a == b) {
                    const Uint = intType(.unsigned, @bitSizeOf(T));
    ...
    fn simdNarrowS(self: *Interpreter, comptime Src: type, comptime Dst: type) TrapError!void {
        ...
        const SrcSigned = intType(.signed, @bitSizeOf(Src));
        const DstSigned = intType(.signed, @bitSizeOf(Dst));
