...
const Reader = struct {
    ...
    fn readElementSection(self: *Reader, end: usize) ReadError!void {
        ...
        for (0..count) |_| {
            ...
            // Function-index element segments have the inferred, non-nullable
            // type `(ref func)`. The binary has no reftype field for these
            // forms, but consumers still need their actual type.
            if (!use_elem_exprs) seg.elem_type = .ref_func;
...
test "function-index element segments infer non-nullable ref func" {
    // Generated by wasm-tools 1.250.0 from a valid module with active,
    // passive, and declared `func x*` element segments. Flags 0, 1, and 3
    // encode no reftype, but all three infer `(ref func)`.
    const allocator = std.testing.allocator;
    ...
    const src = [_]u8{
        ...
        0x04, 0x0a, 0x01, 0x40, 0x00, 0x64, 0x70, 0x00, 0x01, 0xd2, 0x00, 0x0b,
        // active, passive, and declared function-index segments
        0x09, 0x0f, 0x03,
        0x00, 0x41, 0x00, 0x0b, 0x01, 0x00,
        0x01, 0x00, 0x01, 0x00,
        0x03, 0x00, 0x01, 0x00,
        0x0a, 0x1b, 0x02, 0x02, 0x00, 0x0b,
        0x16, 0x00, 0x41, 0x00, 0x41, 0x00, 0x41, 0x00, 0xfc, 0x0c, 0x01, 0x00,
        0x41, 0x00, 0x41, 0x00, 0x41, 0x00, 0xfc, 0x0c, 0x02, 0x00, 0x0b,
    };
    var module = try readModule(allocator, &src);
    defer module.deinit();

    const kinds = [_]types.SegmentKind{ .active, .passive, .declared };
    for (module.elem_segments.items, kinds) |seg, kind| {
        try std.testing.expectEqual(kind, seg.kind);
